...
class QuartileDiscretizer(BaseDiscretizer):
    def __init__(self, data, categorical_features, feature_names, labels=None, n_bins=4):
        self.n_bins = n_bins
        self.gap = int(np.round(100.0 / self.n_bins))
        self.percentiles = np.arange(self.gap, 100.0, self.gap)
        super().__init__(data, categorical_features, feature_names, labels)
...
class UniformDiscretizer(BaseDiscretizer):
    ...
    def bins(self, data, labels):
        bins = []
        for feature in self.to_discretize:
            maxs = np.max(data[:, feature])
            mins = np.min(data[:, feature])
            gap = (maxs - mins) / (self.n_bins - 1)
            if maxs > mins:
                qts = np.array(np.arange(mins, maxs, gap))
            else:
                qts = np.array([mins])
            bins.append(qts)
        return bins
...
class EntropyDiscretizer(BaseDiscretizer):
    def __init__(self, data, categorical_features, feature_names, labels=None, n_bins=8):
        if labels is None:
            raise ValueError('Labels must be not None when using EntropyDiscretizer')
        self.n_bins = n_bins
        self.max_depth = max(1, int(np.ceil(np.log2(self.n_bins))))
        super().__init__(data, categorical_features, feature_names, labels)
    ...
    def bins(self, data, labels):
        ...
        for feature in self.to_discretize:
            ...
            dt = DecisionTreeClassifier(criterion='entropy', max_depth=self.max_depth)
