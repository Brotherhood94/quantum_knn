...
import tests.basis_threshold_tests.test_3 as t3
import tests.basis_threshold_tests.test_4 as t4
...
for enc_type in encodings:
    for d_call, d_name in datasets.items():
        ...
        k_max = 5
        for knn_k in range(1,k_max):
            print("[Started Test 1 PCA]")
            print("K threshold: {0}".format(knn_k))
