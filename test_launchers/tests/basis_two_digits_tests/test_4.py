...
def test_4_pca(X, y, knn_k, test_iters, exp_records_per_class, dataset, n_classes, n_features_real, training_type, test_size, enc_type, n_bits, comb):
    ...
    for n_features_pca in (2**e for e in features_range): #start from 2, exponential step
        for n_records_per_class in (2**e for e in trange(1, exp_records_per_class, desc='n_records_per_class', leave=False)): 
            ...
            print_to_file(dataset+"_TEST_4_PCA:"+str(n_features_pca)+"_n_records_per_class_"+str(n_records_per_class)+"_"+enc_type+"_"+str(comb), None, None, bKNN_exps, bQKNN_exps)
            bQKNN_exps = []
            bKNN_exps = []
            di_test = []
