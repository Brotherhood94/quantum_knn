...
def test_4_no_pca(X, y, knn_k, test_iters, exp_records_per_class, dataset, n_classes, n_features_real, training_type, test_size, comb):
    ...
    for n_records_per_class in (2**e for e in trange(1, exp_records_per_class, desc='n_records_per_class')): 
        ...
        print_to_file(dataset+"_TEST_4_No_PCA_n_records_per_class"+str(n_records_per_class)+"_"+str(comb), KNN_exps, aQKNN_exps)
        aQKNN_exps = []
        KNN_exps = []
        di_test = []
...
def test_4_pca(X, y, knn_k, test_iters, exp_records_per_class, dataset, n_classes, n_features_real, training_type, test_size, comb):
    ...
    for n_features_pca in (2**e for e in features_range): #start from 2, exponential step
        for n_records_per_class in (2**e for e in trange(1, exp_records_per_class, desc='n_records_per_class', leave=False)): 
            ...
            print_to_file(dataset+"_TEST_4_PCA:"+str(n_features_pca)+"_n_records_per_class_"+str(n_records_per_class)+"_"+str(comb), KNN_exps, aQKNN_exps)
            aQKNN_exps = []
            KNN_exps = []
            di_test = []
