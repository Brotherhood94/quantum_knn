...
def test_1_no_pca(X, y, knn_k, test_iters, train_iters, dataset, n_classes, n_features_real, training_type, test_size, comb):
    #TEST 1
    ...
    for random_state_test in trange(0, test_iters, desc='test'): #random state test
        ...
        print_to_file(dataset+"_TEST_1_No_PCA_"+str(comb), KNN_exps, aQKNN_exps, None, None)
        aQKNN_exps = []
        KNN_exps = []
        di_train = []


def test_1_pca(X, y, knn_k, test_iters, train_iters, dataset, n_classes, n_features_real, training_type, test_size, comb):
    ...
    for n_features_pca in (2**e for e in features_range): #start from 2, exponential step
        ...
        for random_state_test in trange(0,test_iters, desc='test', leave=False): #random state test
            ...
            print_to_file(dataset+"_TEST_1_PCA_"+str(comb), KNN_exps, aQKNN_exps, None, None)
            aQKNN_exps = []
            KNN_exps = []
            di_train = []
